...
void remove_selected(GtkButton *button, AppWidgets *app) {
    GtkListBoxRow *row=gtk_list_box_get_selected_row(GTK_LIST_BOX(app->list_box));
    if (row) {
        const char *content=gtk_label_get_text(GTK_LABEL(gtk_bin_get_child(GTK_BIN(row))));
        GList *found_node=g_list_find_custom(app->clipboard_history, content, (GCompareFunc)strcmp);
        if (found_node) {
            app->clipboard_history=g_list_delete_link(app->clipboard_history, found_node);
        }
...
void remove_all(GtkButton *button, AppWidgets *app) {
...
int main(int argc, char *argv[]) {
    ...
    // Buttons
    GtkWidget *button_box_top=gtk_button_box_new(GTK_ORIENTATION_HORIZONTAL);
    GtkWidget *copy_button=gtk_button_new_with_label("Copy");
    GtkWidget *about_button=gtk_button_new_with_label("About");
    gtk_container_add(GTK_CONTAINER(button_box_top), copy_button);
    gtk_container_add(GTK_CONTAINER(button_box_top), about_button);
    gtk_box_pack_start(GTK_BOX(vbox), button_box_top, FALSE, FALSE, 0);

    GtkWidget *button_box_bottom=gtk_button_box_new(GTK_ORIENTATION_HORIZONTAL);
    GtkWidget *remove_button=gtk_button_new_with_label("Remove");
    GtkWidget *remove_all_button=gtk_button_new_with_label("Remove All");
    gtk_container_add(GTK_CONTAINER(button_box_bottom), remove_button);
    gtk_container_add(GTK_CONTAINER(button_box_bottom), remove_all_button);
    gtk_box_pack_start(GTK_BOX(vbox), button_box_bottom, FALSE, FALSE, 0);
    ...
    g_signal_connect(remove_button, "clicked", G_CALLBACK(remove_selected), &app);
    g_signal_connect(remove_all_button, "clicked", G_CALLBACK(remove_all), &app);
