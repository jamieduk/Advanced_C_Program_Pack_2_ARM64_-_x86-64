...
// Function to take a screenshot using gnome-screenshot
void on_screenshot_clicked(GtkWidget *widget, gpointer data) {
    // Using gnome-screenshot to take a screenshot interactively
    if (system("/usr/bin/gnome-screenshot -i") != 0) {
        GtkWidget *error_dialog=gtk_message_dialog_new(
            NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
            "Failed to run gnome-screenshot. Ensure it is installed and available at /usr/bin/gnome-screenshot."
        );
